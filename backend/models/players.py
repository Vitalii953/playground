...
class Player(Base):
    ...
    id: Mapped[
        Annotated[uuid.UUID, mapped_column(primary_key=True, default=uuid.uuid4)]
    ]
    created_at: Mapped[
        Annotated[
    ...
    last_updated: Mapped[
        Annotated[
            datetime, mapped_column(onupdate=func.now(), server_default=func.now())
        ]
    ]

    gold: Mapped[Annotated[int, mapped_column(default=0)]]
    keys: Mapped[Annotated[int, mapped_column(default=0)]]
    current_floor: Mapped[
        Annotated[int, mapped_column(default=0)]
